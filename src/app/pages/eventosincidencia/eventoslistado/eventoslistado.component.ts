...
import { EventosIncidencias } from 'src/app/models/eventosincidencia';
import { NavigationEnd, Router } from '@angular/router';

@Component({
  selector: 'app-eventoslistado',
  templateUrl: './eventoslistado.component.html',
})
export class EventoslistadoComponent implements OnInit, OnDestroy {
  ...
  flag: EventosIncidencias = new EventosIncidencias();
  ...
  constructor(
    ...
    */
    this.eventosService.getListado((data) => {
      this.temp = [...data];
      this.rows = data;
      this.user = data[0]['id_persona'];
      this.flag = data[0]['gee_flag'];
      this.dataService
  ...
  updateFilter(event) {
    ...
    const temp = this.temp.filter(function (d) {
      if (d && d.gee_fecha_inicio && d.gee_contenido && d.gee_descripcion) {
        return (
          d.gee_fecha_inicio.toLowerCase().indexOf(val) !== -1 ||
          d.gee_contenido.toLowerCase().indexOf(val) !== -1 ||
  ...
  guardarEvento() {
    ...
    if (this.eventosForm.valid) {
      ...
      this.eventosService.guardarNuevoEvento(valor).subscribe((respuesta) => {
        Swal.fire({
